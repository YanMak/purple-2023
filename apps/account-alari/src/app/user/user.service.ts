import { Injectable } from '@nestjs/common';
import { IUser_Alari } from '@purple-2023/interfaces';
import { RMQService } from 'nestjs-rmq';
import { UserEntity_Alari } from './entities/user.entity';
import { UserRepository_Alari } from './repositories/user.repository';
import { BuyCourseSaga_Alari } from './sagas/buy-course.saga';
import { UserEventEmiiter } from './user.event-immiter';

@Injectable()
export class UserService {
	constructor(
		private readonly userRepository: UserRepository_Alari,
		private readonly rmqService: RMQService,
		private readonly userEventEmmiter: UserEventEmiiter
	) { }

	public async changeProfile(user: Pick<IUser_Alari, 'displayName'>, id: string) {
		const existedUser = await this.userRepository.findUserById(id);
		if (!existedUser) {
			throw new Error('Такого пользователя не существует');
		}
		const userEntity = new UserEntity_Alari(existedUser).updateProfile(user.displayName);
		await this.updateUser(userEntity);
		return {};
	}

	public async buyCourse(userId: string, courseId: string) {
		const existedUser = await this.userRepository.findUserById(userId);
		if (!existedUser) {
			throw new Error('Такого пользователя нет');
		}
		const userEntity = new UserEntity_Alari(existedUser);
		const saga = new BuyCourseSaga_Alari(userEntity, courseId, this.rmqService);
		const { user, paymentLink } = await saga.getState().pay();
		await this.updateUser(user);
		return { paymentLink };
	}

	public async checkPayments(userId: string, courseId: string) {
		const existedUser = await this.userRepository.findUserById(userId);
		if (!existedUser) {
			throw new Error('Такого пользователя нет');
		}
		const userEntity = new UserEntity_Alari(existedUser);
		const saga = new BuyCourseSaga_Alari(userEntity, courseId, this.rmqService);
		const { user, status } = await saga.getState().checkPayment();
		await this.updateUser(user);
		return { status };
	}

	private updateUser(user: UserEntity_Alari) {
		return Promise.all([
			this.userEventEmmiter.handle(user),
			this.userRepository.updateUser(user)
		]);
	}
}