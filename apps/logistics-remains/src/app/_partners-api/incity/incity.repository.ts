...
export class WarehouseParametersRepository_Partner {
  constructor(
    @InjectModel(WarehouseParameters_Partner_Incity.name)
    private readonly warehouseParametersModel_Partner: Model<WarehouseParameters_Partner_Incity>
  ) {}

  async createUpdate(point: IWarehouseParametersEntity_Partner_Incity) {
    const { id, ...rest } = point;
    const existedPoint = await this.warehouseParametersModel_Partner
      .findOne({ id })
      .exec();
    if (!existedPoint) {
      ...
      const res = await this.warehouseParametersModel_Partner.updateOne(
        { id },
        { $set: rest }
      );
      return res;
