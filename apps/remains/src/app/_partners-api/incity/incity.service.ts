...
import { IWarehouseParameters_Partner_Incity, IWarehouseRemains_Partner_Incity_V1 } from './incity.interface';
...
export class IncityService {
	...
	public async getPartnerWarehousesPropertiesFromERP(): Promise<IWarehouseParameters_Partner_Incity[]> {
		...
		const { data } = await this.httpService.axiosRef.get<unknown, AxiosResponse<IWarehouseParameters_Partner_Incity[]>>('http://corp.incity.ru:88/OnlineStore/hs/ecom_api_trade/v1/get_stores_v1?count=1');
		//console.log(data)
		return data
		//const 
	}
	...
	public async createUpdatePointsProfiles() {
		...
		const connectUrl = 'http://' + apiUrl + ':3334/remains/create-update-points'
	...
	public async createUpdatePartnerWarehouseProfile(point: IWarehouseParameters_Partner_Incity) {
	...
	public async reloadRetailWarehouseRemains(point: IWarehouseParameters_Partner_Incity) {
		try {
			console.log(`trying get remains ` + point.title + ' ' + point.outerURL);
			const pointRemainsERP = await this.getWarehouseRemainsFromERP('http://' + point.outerURL);
			const pointRemains = pointRemainsERP.map(remain => ({ id: remain.Штрихкод, barcode: remain.Штрихкод, count: remain.КоличествоОстаток }))
			//console.log(pointRemains)
			//const res = await this.pointRepository.setRemains(id, pointRemains);
			console.log(`succesfully get remains ` + point.title + ' ' + point.outerURL);
			//return res
			...
			//console.log(connectUrl);
			const res = await this.httpService.axiosRef.post(
				connectUrl, { id: point.id, remains: pointRemains }
			);
			return res
		}
		catch (e) {
	...
	public async getWarehouseRemainsFromERP(outerUrl: string): Promise<IWarehouseRemains_Partner_Incity_V1[]> {
		//public async getWarehouseRemainsFromERP(outerUrl: string): Promise<void> {
		const { data } = await this.httpService.axiosRef.get<unknown, AxiosResponse<IWarehouseRemains_Partner_Incity_V1[]>>(outerUrl);
